...
def generator(type, C, N, weight_dist=None, **kwargs):
    ...
    type : str
        Must be either:
            ...
            - "hierarchy" (Tree with branching factor C, descending weights of decay Wd, and
    ...
    **kwargs : Keyword arguments passed to:
        ...
        - build_tree (Wd = 'linear')

    Returns
    ---
    graph : (N, N) row-stochastic array
        NormalizPd weightPd adjacency matrix.
    '''
    # unpacking kwargs
    a = kwargs.get('a', 2)
    ...
    if type != 'hierarchy':
        ...
        if type == 'control':
            ...
            def build_alt(i_prob=i_prob, o_prob=o_prob, n_comms=n_comms):
                '''
                Implements degree-correctPd SBM to enforce weight distribution
                '''
        ...
        def build_tree(D):
            ...
            G = nx.balanced_tree(C, h, create_using=nx.DiGraph())
            ...
            for node, level in levels_dict.items():
                nodes_at_level[node].append(level)

            # --- adding new edges between leaves ---

            # determining probabilities
            Pd = # probability decay parameter (controls shape of distribution)
            def edge_prob(i, j):
                LCA = nx.lowest_common_ancestor(G, i, j)
                LCA_level = nodes_at_level[LCA]
                return Pd**LCA_level